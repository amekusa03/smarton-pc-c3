...
*   **Ping戦略**:
    シーンによってIPアドレス指定とmDNSを使い分ける。

    | シーン | 手段 | 理由 |
    | :--- | :--- | :--- |
    | OFF→ON（起動確認） | mDNSのみ（`hostname.local`） | 起動直後はIPが不定のため |
    | ON中の死活監視（通常） | IPアドレスで高速ポーリング | 既知のIPで低レイテンシ |
    | ON中の死活監視（無応答時） | mDNSで再確認 | IPが変わった誤検知を排除し確定判断する |

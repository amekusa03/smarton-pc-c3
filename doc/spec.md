...
*   **PCの状態検知**:
    ...
    *   Pingに応答なし かつ 内部状態が `IDLE` → **OFF またはフリーズ**（死活監視の対象）
...
*   **ON命令受信時**:
    ...
    3. **起動待機時間（60秒 ※実測値により変更する）** が経過したら内部状態を `IDLE` に戻し、Ping監視を再開する。
    4. 待機時間内にPing応答があれば即座に `IDLE` へ移行してよい（確認失敗は許容）。
...
*   **死活監視（フリーズ検知・回復）**:
    *   内部状態が `IDLE` のときのみ実施する。
    ...
    4. 強制電源断後は内部状態を `BOOTING` に設定し、再起動を待つ。
    5. Google Homeへ通知する（任意）。
